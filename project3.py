# -*- coding: utf-8 -*-
"""
Created on Sun Dec  8 16:34:15 2019
...
root.geometry(
    "400x150+" + str(int(screen_width - 300 / 2)) + "+" + str(int(screen_height - 150 / 2)))
...
def Add_Employee():
    ...
    def Add_Employee_submit():
        creat_tabile()
        c.execute("INSERT INTO allEmp VALUES ({},'{}','{}','{}','{}','{}','{}',{})".format(
            EmployeeNumber.get(), EmployeeName.get(), EmployeeGender.get(),
            EmployeeNationality.get(), EmployeeDateofBirth.get(), EmployeeAddress.get(),
            EmployeeDepartment.get(), EmployeeSalary.get()))
        conn.commit()
        print("upload done")
    ...
    e04 = Radiobutton(add_em_sc, text="male", padx=20, variable=EmployeeGender, value="male").grid(
        row=2, column=1)
    e04 = Radiobutton(add_em_sc, text="female", padx=20, variable=EmployeeGender,
                      value="female").grid(row=3, column=1)
...
def view_Employee():
    ...
    view_em_sc.geometry(
        "600x600+" + str(int((screen_width - 300) / 4)) + "+" + str(int((screen_height - 150) / 4)))
